...
def test_space_travel_benefits(browser):
    ...
    assert padding_top_of(travel_benefits_row, '40px') and padding_bottom_of(travel_benefits_row, '20px'), 'Padding top or bottom is wrong'
...
def test_space_feature_solution(browser):
    ...
    assert padding_top_of(feature_solution_row, '30px'), 'Padding top is incorrect, the correct value is 30px'
...
def test_space_sneak_peek(browser):
    ...
    for i, card in enumerate(cards):
        assert margin_bottom_of(card, '20px'), 'Margin bottom of card is wrong'
